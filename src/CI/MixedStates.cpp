...
//==============================================================================
// Solves [<I|H|J> - E_0 d_IJ] c_J = -<I||T||K> c_K, for the mixed state c_I
PsiJPi solve_mixed_state(const PsiJPi &Psi0, std::size_t i0,
  ...
  // Left-hand side: M_IJ = <I|H|J> - E_0 * delta_IJ
  auto M = Hci;
  for (std::size_t i = 0; i < N; ++i) {
    M(i, i) -= E0;
  }

  // If target sector is that of Psi_0, then M is singular (M|Psi_0> = 0).
  // Project Psi_0 out of the right-hand side, and add |Psi_0><Psi_0| to M,
  // which leaves M unchanged in the orthogonal subspace, but makes it
  // invertible; the solution is then orthogonal to Psi_0
  if (target.twoJ() == Psi0.twoJ() && target.parity() == Psi0.parity()) {
...
PsiJPi solve_mixed_state(const PsiJPi &Psi0, std::size_t i0, int twoJ,
  ...
  return solve_mixed_state(Psi0, i0, target, Hci, h, K_rank);
}
