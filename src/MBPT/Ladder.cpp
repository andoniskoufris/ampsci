...
double L2_CC_new(int k, const DiracSpinor &m, const DiracSpinor &n,
  ...
  for (auto ir = 0ul; ir < excited_size; ++ir) {
    ...
    for (auto ic = 0ul; ic < core_size; ++ic) {
      ...
      // // Cache (Q+L)^l_rmic: depends only on l, used inside the u loop (see L1).
      // assert(lI < int(sk_array_size));
      // std::array<double, sk_array_size> QLl_rmic{};
      // for (auto l = l0; l <= lI; l += 1) {
      //   QLl_rmic[std::size_t(l)] =
      //     qk.Q(l, Qkey_rmic) + (Lk ? Lk->Q(l, Lkey_rmic) : 0.0);
      // }
      ...
      auto PM_kmric =
        qk.P(k, m, r, i, c, &SJ) + (Lk ? Lk->P(k, m, r, i, c, &SJ) : 0.0);
      ...
      const auto inv_e_cimr = 1.0 / (c.en() + eim - r.en());
      l2 +=
        P_kcnrj * PM_kmric * inv_e_cimr / ((2.0 * k + 1.0) * (2.0 * k + 1.0));
    }
  }
