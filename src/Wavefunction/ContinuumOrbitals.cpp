...
void ContinuumOrbitals::IncludeExchange(DiracSpinor &Fc, const DiracSpinor *Fi,
  ...
  for (int it = 0; it <= max_its; ++it) {
    ...
    // For HF, use KS as local exchange approx.
    // Only use 'vex_approx' for that specific method
    const auto vx0 = p_hf->method() == HF::Method::ApproxHF ?
                       HF::vex_approx(Fc, p_hf->core()) :
                       HF::vex_KS(p_hf->core());
