...
DiracSpinor Breit::BPkv_bcd(int k, int kappa_v, const DiracSpinor &Fb,
  ...
  return BPkv_bcd_freqw(k, kappa_v, Fb, Fc, Fd, 0.0);
}
...
DiracSpinor Breit::BWkv_bcd(int k, int kappa_v, const DiracSpinor &Fb,
                            const DiracSpinor &Fc,
                            const DiracSpinor &Fd) const {
  return BWkv_bcd_freqw(k, kappa_v, Fb, Fc, Fd, 0.0);
}
...
double Breit::BWk_abcd(int k, const DiracSpinor &Fa, const DiracSpinor &Fb,
                       const DiracSpinor &Fc, const DiracSpinor &Fd) const {
  return Bk_abcd(k, Fa, Fb, Fc, Fd) + BPk_abcd(k, Fa, Fb, Fc, Fd);
}
...
//==============================================================================
double Breit::Bk_abcd_freqw(int k, const DiracSpinor &Fa, const DiracSpinor &Fb,
                            const DiracSpinor &Fc,
                            const DiracSpinor &Fd) const {
  ...
  return wbd == wac ?
           Fa * Bkv_bcd_freqw(k, Fa.kappa(), Fb, Fc, Fd, wbd) :
           0.5 * (Fa * (Bkv_bcd_freqw(k, Fa.kappa(), Fb, Fc, Fd, wbd) +
                        Bkv_bcd_freqw(k, Fa.kappa(), Fb, Fc, Fd, wac)));
}

//==============================================================================
double Breit::BPk_abcd_freqw(int k, const DiracSpinor &Fa,
                             const DiracSpinor &Fb, const DiracSpinor &Fc,
                             const DiracSpinor &Fd) const {

  // XXX Is this correct? Double check!
  ...
  return wad == wbc ?
           Fa * BPkv_bcd_freqw(k, Fa.kappa(), Fb, Fc, Fd, wad) :
           0.5 * (Fa * (BPkv_bcd_freqw(k, Fa.kappa(), Fb, Fc, Fd, wad) +
                        BPkv_bcd_freqw(k, Fa.kappa(), Fb, Fc, Fd, wbc)));
}

//==============================================================================
double Breit::BWk_abcd_freqw(int k, const DiracSpinor &Fa,
                             const DiracSpinor &Fb, const DiracSpinor &Fc,
                             const DiracSpinor &Fd) const {
  return Bk_abcd_freqw(k, Fa, Fb, Fc, Fd) + BPk_abcd_freqw(k, Fa, Fb, Fc, Fd);
}
...
double Breit::BPk_abcd_freqw(int k, const DiracSpinor &Fa,
                             const DiracSpinor &Fb, const DiracSpinor &Fc,
                             const DiracSpinor &Fd, const double w) const {
...
double Breit::BWk_abcd_freqw(int k, const DiracSpinor &Fa,
                             const DiracSpinor &Fb, const DiracSpinor &Fc,
                             const DiracSpinor &Fd, const double w) const {
  return Bk_abcd_freqw(k, Fa, Fb, Fc, Fd, w) +
         BPk_abcd_freqw(k, Fa, Fb, Fc, Fd, w);
}
...
DiracSpinor Breit::BPkv_bcd_freqw(int k, int kappa_v, const DiracSpinor &Fb,
  ...
  for (int twol = min_twol; twol <= max_twol; twol += 2) {
    ...
    if (sj == 0.0)
      continue;
    out += sj * Bkv_bcd_freqw(twol / 2, kappa_v, Fb, Fd, Fc, w);
  }
  out *= (2.0 * k + 1.0);
  return out;
}
...
//==============================================================================
DiracSpinor Breit::BWkv_bcd_freqw(int k, int kappa_v, const DiracSpinor &Fb,
                                  const DiracSpinor &Fc, const DiracSpinor &Fd,
                                  const double w) const {
  return Bkv_bcd_freqw(k, kappa_v, Fb, Fc, Fd, w) +
         BPkv_bcd_freqw(k, kappa_v, Fb, Fc, Fd, w);
}
